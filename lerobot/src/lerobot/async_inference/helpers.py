...
class FPSTracker:
    """
    统计从开始到当前时刻的平均FPS（推理服务每秒处理的请求数）
    """

    # 目标FPS（需外部设置，比如期望每秒处理30个推理请求）
    target_fps: float
    # 第一次统计的时间戳（初始化时为None）
    ...
    def calculate_fps_metrics(self, current_timestamp: float) -> dict[str, float]:
        ...
        # 每次调用，累计计数+1（代表处理了1个任务/帧）
        self.total_obs_count += 1

        # 初始化第一次统计的时间戳（仅第一次调用时执行）
        ...
        # 计算平均FPS：(总次数-1) / 总时长（减1是因为第一次调用时仅初始化时间，无有效时长）
        # 加 1e-6 是避免除以0（当总时长极小时）
